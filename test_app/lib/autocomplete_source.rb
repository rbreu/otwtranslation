    ...
    def autocomplete_lookup(options = {})
      ...
      search_pieces.each_with_index do |search_piece, index|
        ...
        if index == search_pieces.size - 1
          lastpiece = true
          search_piece.gsub!(/#{AUTOCOMPLETE_WORD_TERMINATOR}$/, '')
        ...
        completions.each do |word|
          ...
          if lastpiece && search_piece.length < 3
            # use a limit
            phrases_with_scores = $redis.zrevrangebyscore(autocomplete_score_key(autocomplete_prefix, word), 'inf', 0, :withscores => true, :limit => [0, 50])
          else
            phrases_with_scores = $redis.zrevrangebyscore(autocomplete_score_key(autocomplete_prefix, word), 'inf', 0, :withscores => true)
          end
